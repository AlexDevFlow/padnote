...
#include <vector>
#include <assert.h>
#include <cstring>     // Phase 5U.1 — strcmp / strlen (was implicit via <windows.h>)
// Phase 5U.1 (Linux port) — <windows.h> include was vestigial for everything
// except a handful of CRT helpers. The body of this file uses only
// <string>/<map>/<vector>/<assert.h>/<cstring> + Lexilla's own headers; no
// Win32 types appear. <cstring> replaces what windows.h was implicitly
// dragging in; the _itoa() replacement below handles the only Win32-CRT
// function the original code depended on.
...
// Phase 5U.1 (Linux port) — portable replacement for Win32's _itoa with
// radix=10. Writes the signed decimal representation of `value` followed
// by a null terminator into `dest`. No bounds checking — same contract as
// upstream _itoa. The original call sites (in ColouriseUserDoc) write at
// fixed offsets into a stack buffer sized for at most 2-digit numbers,
// which all SCE_USER_STYLE_* values fit; replacement preserves those
// assumptions byte-for-byte.
static inline void itoa10(int value, char* dest)
{
    if (value < 0) {
        *dest++ = '-';
        value = -value;
...
static void ColouriseUserDoc(Sci_PositionU startPos, Sci_Position length, int initStyle, WordList *kwLists[], Accessor &styler)
{
    ...
    itoa10(SCE_USER_STYLE_COMMENT,       (nestingBuffer+20));    int commentNesting      = styler.GetPropertyInt(nestingBuffer, 0);
    itoa10(SCE_USER_STYLE_COMMENTLINE,   (nestingBuffer+20));    int lineCommentNesting  = styler.GetPropertyInt(nestingBuffer, 0);
    itoa10(SCE_USER_STYLE_DELIMITER1,    (nestingBuffer+19));    int delim1Nesting       = styler.GetPropertyInt(nestingBuffer, 0);  // one byte difference
    itoa10(SCE_USER_STYLE_DELIMITER2,    (nestingBuffer+19));    int delim2Nesting       = styler.GetPropertyInt(nestingBuffer, 0);  // for two-digit numbers
    itoa10(SCE_USER_STYLE_DELIMITER3,    (nestingBuffer+19));    int delim3Nesting       = styler.GetPropertyInt(nestingBuffer, 0);
    itoa10(SCE_USER_STYLE_DELIMITER4,    (nestingBuffer+19));    int delim4Nesting       = styler.GetPropertyInt(nestingBuffer, 0);
    itoa10(SCE_USER_STYLE_DELIMITER5,    (nestingBuffer+19));    int delim5Nesting       = styler.GetPropertyInt(nestingBuffer, 0);
    itoa10(SCE_USER_STYLE_DELIMITER6,    (nestingBuffer+19));    int delim6Nesting       = styler.GetPropertyInt(nestingBuffer, 0);
    itoa10(SCE_USER_STYLE_DELIMITER7,    (nestingBuffer+19));    int delim7Nesting       = styler.GetPropertyInt(nestingBuffer, 0);
    itoa10(SCE_USER_STYLE_DELIMITER8,    (nestingBuffer+19));    int delim8Nesting       = styler.GetPropertyInt(nestingBuffer, 0);
